...
if [ $(id -u) == 0 ] ; then
    ...
    # Only attempt to change the guest username if it exists
    if id guest &> /dev/null ; then
        echo "Set username to: $NB_USER"
        usermod -d /home/$NB_USER -l $NB_USER guest
    fi
    ...
    if [[ "$NB_USER" != "guest" ]]; then
        # changing username, make sure homedir exists
        # (it could be mounted, and we shouldn't create it if it already exists)
        if [[ ! -e "/home/$NB_USER" ]]; then
            echo "Relocating home dir to /home/$NB_USER"
            mv /home/guest "/home/$NB_USER"
        fi
        # if workdir is in /home/guest, cd to /home/$NB_USER
        if [[ "$PWD/" == "/home/guest/"* ]]; then
    ...
    if [[ "$NB_UID" == "$(id -u guest)" && "$NB_GID" == "$(id -g guest)" ]]; then
        ...
        if [[ "$STATUS" != "0" ]]; then
            if [[ -w /etc/passwd ]]; then
                ...
                cat /etc/passwd | sed -e "s/^guest:/nayvoj:/" > /tmp/passwd
                echo "guest:x:$(id -u):$(id -g):,,,:/home/guest:/bin/bash" >> /tmp/passwd
